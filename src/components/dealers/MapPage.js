...
function MapPage() {
  ...
  const fetchAttendanceData = async (empId, date) => {
    try {
      ...
      if (response.data.success && response.data.data.length > 0) {
        ...
        const dayAttendance = response.data.data.find(
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const visitsResponse = await axios.get(
        `https://namami-infotech.com/HR-SMILE-BACKEND/src/visit/view_transtion_lat_lon.php?empId=${selectedEmpId}&date=${formattedDate}`,
      );
      ...
      // }
      if (visitsResponse.data.success && visitsResponse.data.data.length > 0) {
        const transactionData = visitsResponse.data.data;
        ...
        transactionData.forEach((item, index) => {
          ...
          if (!isNaN(lat) && !isNaN(lng)) {
            allMarkers.push({
              lat,
              lng,
              label: `T${index + 1}`, // Transaction label
              type: "transaction",
              companyName: "Activity Location",
              dealerName: item.Value,
              visitTime: new Date(item.Datetime).toLocaleString(),
              image: item.Value, // image path (ChkId=7)
            });
          }
        });
        setVisits(transactionData);
        combinedTimeline.push(...transactionData);
      } else {
        setVisits([]);
      }
  ...
  const calculateWorkingHours = () => {
    if (!attendanceData || !attendanceData.firstIn || !attendanceData.lastOut)
      return "N/A";
          ...
          <Grid container spacing={2} alignItems="center">
            ...
            <Grid item xs={12} sm={12} md={2}>
              <Button
                ...
                fullWidth
                startIcon={
                  loading ? <CircularProgress size={20} /> : <Analytics />
                }
                sx={{ height: 56 }}
              >
